...
import 'askar_string_repository.dart';
...
class StoreRepository {
  ...
  Future<bool> createProfile() async {
    checkStore();
    final result = await askarStoreCreateProfile(
      StoreHandle(handle!.toInt()),
      profile: "${profile}2",
    );
  ...
  Future<bool> rekey(String newPassKey) async {
    checkStore();
    final result = await askarStoreRekey(
      StoreHandle(handle!.toInt()),
      keyMethod: method,
      passKey,
    );
  ...
  Future<bool> setDefaultProfile(String profileName) async {
    checkStore();
    final result = await askarStoreSetDefaultProfile(
      StoreHandle(handle!.toInt()),
      profileName,
    );
    if (result.errorCode == ErrorCode.success) {
