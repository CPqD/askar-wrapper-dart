...
void main() {
  group('Askar Tests', () {
    ...
    test('Key From Seed Test', () async {
      ...
      final resultKey1None = keyFromSeedTest(
        KeyAlgorithm.bls12381G1,
        seed1,
        KeyMethod.none,
      );
      final resultKey2None = keyFromSeedTest(KeyAlgorithm.bls12381G1, seed1);
      final resultKey3Bls = keyFromSeedTest(
        KeyAlgorithm.bls12381G1,
        seed1,
        KeyMethod.blsKeygen,
      );
      final resultKey4Bls = keyFromSeedTest(
        KeyAlgorithm.bls12381G1,
        seed1,
        KeyMethod.blsKeygen,
      );
    ...
    test('Askar Key Wrap and Unwrap', () async {
      ...
      expect(
        ...
        equals('Az2IWsISEMDJvyc5XRL-3-d-RgNBOGolCsxFFoUXFYw'),
      );
      ...
      final derived =
          keyDeriveEcdh1puTest(
            KeyAlgorithm.aesA128Kw,
            ephemeral,
            alice,
            bob,
            utf8.encode(alg),
    ...
    test('Get Key From Secret Bytes', () async {
      ...
      final keyFromSecretBytesResult = keyFromSecretBytesTest(
        KeyAlgorithm.aesA256CbcHs512,
        secret,
      );
    ...
    test('Removing Key', () async {
      ...
      final fetchKeyResult = await sessionFetchKeyTest(
        sessionHandle,
        name,
        expectSuccess: true,
      );
  ...
  group('Key Aead Decryption Tests:', () {
    test('Decryption with and without aad', () {
      ...
      keyAeadDecryptTest(
        localKeyHandle,
        encryptedBuffer.value.ciphertext,
        nonce.value,
        tag: encryptedBuffer.value.tag,
        aad: aad,
        expected: message,
      );
      keyAeadDecryptTest(
...
AskarResult<bool> keyGetEphemeralTest(
...
AskarResult<EncryptedBuffer> keyAeadEncryptTest(
  LocalKeyHandle handle,
  Uint8List message, {
  Uint8List? nonce,
  Uint8List? aad,
...
Future<AskarResult<EntryListHandle>> scanNextTest(ScanHandle handle) async {
...
AskarResult<LocalKeyHandle> keyGenerateTest(
  KeyAlgorithm algorithm, {
  KeyBackend keyBackend = KeyBackend.software,
...
AskarResult<EncryptedBuffer> keyWrapKeyTest(
  LocalKeyHandle handle,
...
AskarResult<String> keyGetAlgorithmTest(
  LocalKeyHandle handle, {
...
AskarResult<Uint8List> keyGetSecretBytesTest(
  LocalKeyHandle handle, {
  required Uint8List expected,
}) {
...
AskarResult<String> keyEntryListGetNameTest(
...
AskarResult<Map> keyEntryListGetTagsTest(
  KeyEntryListHandle handle,
  int index, {
...
AskarResult<String> keyEntryListGetAlgorithmTest(
  KeyEntryListHandle handle,
  int index, {
  required KeyAlgorithm expected,
}) {
...
AskarResult<int> entryListCountTest(
...
AskarResult<int> keyEntryListCountTest(
  KeyEntryListHandle handle, {
  required int expectedValue,
}) {
...
Future<AskarResult<KeyEntryListHandle>> sessionFetchAllKeysTest(
  SessionHandle handle,
  KeyAlgorithm algorithm,
  String thumbprint,
  Map tagFilter,
...
Future<AskarResult<EntryListHandle>> sessionFetchTest(
  SessionHandle handle, {
  bool expectSuccess = true,
}) async {
  String category = 'category-one';
  ...
  AskarResult<EntryListHandle> result = AskarResult<EntryListHandle>(
    ErrorCode.custom,
    EntryListHandle(0),
  );
...
AskarResult<Uint8List> entryListGetValueTest(
  EntryListHandle handle,
  int index, {
...
AskarResult<bool> keyVerifySignatureTest(
  LocalKeyHandle handle,
  Uint8List message,
  Uint8List signature,
  SignatureAlgorithm sigType, {
...
AskarResult<LocalKeyHandle> keyFromSeedTest(
  KeyAlgorithm algorithm,
  Uint8List seed, [
...
AskarResult<int> stringListCountTest(
...
AskarResult<String> stringListGetItemTest(
  StringListHandle handle,
  int index, {
...
Future<AskarCallbackResult> storeGetDefaultProfileTest(
  StoreHandle handle, {
  required String expectedValue,
}) async {
...
AskarResult<Uint8List> keyCryptoBoxOpenTest(
  LocalKeyHandle recipKey,
  LocalKeyHandle senderKey,
  Uint8List message,
  Uint8List nonce,
