...
import '../../src/askar_callbacks.dart';
import '../../src/crypto/encrypted_buffer.dart';
import '../../src/crypto/handles.dart';
import '../../src/enums/askar_entry_operation.dart';
import '../../src/enums/askar_error_code.dart';
import '../../src/enums/askar_key_algorithm.dart';
import '../../src/enums/askar_key_backend.dart';
import '../../src/enums/askar_signature_algorithm.dart';
import '../../src/enums/askar_store_key_method.dart';
...
ErrorCode askarSetCustomLogger(
  ...
  final result = nativeAskarSetCustomLogger(context, log, enabled, flush, maxLevel);
...
AskarResult<String> askarKeyEntryListGetAlgorithm(KeyEntryListHandle handle, int index) {
  ...
  try {
    final result = nativeAskarKeyEntryListGetAlgorithm(
      handle.toInt(),
      index,
      utf8PtPointer,
    );
...
AskarResult<Uint8List> askarKeyGetPublicBytes(LocalKeyHandle handle) {
  ...
  try {
    final funcResult = nativeAskarKeyGetPublicBytes(handle.toInt(), secretBufferPtr);
...
AskarResult<Uint8List> askarKeyGetSecretBytes(LocalKeyHandle handle) {
  ...
  try {
    final funcResult = nativeAskarKeyGetSecretBytes(handle.toInt(), secretBufferPtr);
...
Future<AskarCallbackResult> askarStoreGetDefaultProfile(StoreHandle handle) async {
  final callback = newCallbackWithPtrUtf8();
...
Future<AskarResult<String>> askarStoreGetProfileName(StoreHandle handle) async {
  final callback = newCallbackWithPtrUtf8();
...
Future<AskarCallbackResult> askarStoreRemove(String specUri) async {
  Pointer<Utf8> specUriPointer = nullptr;
